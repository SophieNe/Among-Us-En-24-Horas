...
namespace Mirror
{
    ...
    {
        public static readonly ILogger logger = LogFactory.GetLogger(typeof(NetworkRoomManager));
        ...
        internal void NameStatusChanged(NetworkRoomPlayer play)
        {
            foreach (NetworkRoomPlayer item in roomSlots)
            {
                if (item != null)
                {
                    if (item == play)
                    {
                        item.playerName = play.playerName;
                    }
                }
            }
        }
